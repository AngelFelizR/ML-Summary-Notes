...
# Defining the MODELS and METRICS to evaluate

evaluate_models <- function(recipe,
                            split){
  
  seq.int(10L, 100L, 10L) |>
  c(1L) |>
  ...
  lapply(function(model, recipe, split){
    workflow() |>
      add_recipe(recipe) |>
      add_model(model) |>
  ...
  recipe = recipe,
  split = split) |>
  (\(list.DT) lapply(seq_along(list.DT), function(tb_i){
              list.DT[[tb_i]][, model_name := names(list.DT)[tb_i] ] })
  )() |>
  rbindlist() |>
  dcast(... ~ .metric, value.var = ".estimate")
  
}


# Defining the RECIPES to use
...
WeeklyEvaluationResults <-
  list("BaseFormula" = RecipeBaseFormula,
       "LogVolume" = RecipeLogVolume,
       "Interactions" = RecipeInteractions) |>
  lapply(evaluate_models, split = WeeklyYearSplit) |>
  (\(list.DT) lapply(seq_along(list.DT), function(tb_i){
    list.DT[[tb_i]][, recipe := names(list.DT)[tb_i] ] })
  )() |>
  rbindlist() |>
  DT(order(-roc_auc)) |>
  DT(j = k := sub(pattern = "[A-Za-z]+ ?", replacement = "", model_name) |> as.integer())
 

WeeklyEvaluationResults[, .SD[1L], 
                        by = .(model_name = fifelse(model_name %like% "knn", 
                                                    "knn", model_name))]
...
- In this case the *Logistic Regressions* has the best general resultas after adding interactions to the model.

```{r}
WeeklyFittedWorkFlow <-
  workflow() |>
  add_recipe(RecipeInteractions) |>
