...
- The **singular value decomposition (SVD)** algorithm breaks a matrix into three other matrices, in a way that can be taken back the next function matrix equation $\mathbf{X}= \mathbf{UDV}′$, where:
  - **D**: Is a vector that measure the amount of **variance** of each principal component.
  - **V**: Is a matrix of the right singular vector (also known as **loading vectors**). These vectors are the principal components and represent the directions of maximum variance in the data.
...
index_na <- rbind(row_na, col_na)

arrests_na[index_na] <- NA_real_

```

- Writing a function that returns an approximation to the matrix
...
Algorithm 12.1 steps:

1. Substitute missing values with the mean of each column.

```{r}
arrests_hat <- arrests_na
arrests_hat[index_na] <- colMeans(arrests_na, na.rm = TRUE)[col_na]
```

Computing the first $M$ principal components of the data
