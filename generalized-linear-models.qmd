...
![](img/25-cp-bic-adj-r2-example.png){fig-align="center" width=60% height=60%}
...
The method rather than using RSS as the metric to minimize with the regression coefficient $\beta_0, \beta_1, \dots, \beta_p$, it is modified by adding a **shrinkage penalty** that the effect of estimating $\beta_j$ towards zero when the coefficient is closed to 0:
...
We can think of the x-axis in the right-hand panel of the next figure as the amount that the ridge regression coeﬃcient estimates have been shrunken towards zero; a small value indicates that they have been shrunken very close to zero.

![](img/26-ridge-regression-change.png){fig-align="center" width=60% height=60%}

As the $\lambda$ to select is sensible to the predictors escaling, it's a good practice scaling the predictors using the next formula, to make all the predictors to have an standard deviation of 1:
