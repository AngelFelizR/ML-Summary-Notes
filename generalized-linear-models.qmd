...
|Method|Formula|Description|Interpretation
|:----:|:-----:|:----------|:----------|
|$C_p$|$C_p = \frac{1}{n} (\text{RSS} + 2d\hat{\sigma}^2)$|Where: <br> - *d*: Number of predictors <br> - $\hat{\sigma}^2$: Estimate of the variance of the error $\epsilon$ associated with each response|$2d\hat{\sigma}^2$ represent the penalty of adding new predictors. This method is valid one if our $\hat{\sigma}^2$ is an unbiased estimate of $\sigma^2$|
