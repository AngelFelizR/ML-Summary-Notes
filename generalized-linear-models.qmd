...
It projects the *p* predictors into an *M*-dimensional subspace used to ﬁt a linear regression model by least squares.

Let $Z_1, Z_2, \dots, Z_M$ represent a $M < p$ lineal combinations $Z_m = \sum_{j=1}^p \phi_{jm}X_j$ of our original predictors based on some constants $\phi_{1m}, \phi_{2m}, \dots, \phi_{pm}$ that could lead to improve the fitting against the original model
...
To select the $\phi_{jm}$'s we will discuss two different approachs:
...
By using PCA to perform our analysis we are assuming that *the directions in which* $X_1, \dots, X_p$ *show the most variation are the directions the most variation are the directions* $Y$. If it's that is true then fitting the model to $Z_1, \dots, Z_2$ will lead better results than using the original variables $X_1, \dots, X_p$
