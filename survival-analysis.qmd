...
To achieve this goal we need to overcome that:

1. Some of the observations are censored.
2. The model rather than predicting a single survival time given a covariate vector x, estimates an entire survival curve, $S(t|x)$, as a function of $t$.

To solve those problems we:

1. Stratify the observations using the coefficient estimates and **compute the "risk" score for each test observation**.

$$
\text{budget}_i \cdot \hat{\beta}_{\text{budget}} + \text{impact}_i \cdot \hat{\beta}_{\text{impact}}
$$

2. Categorize the observations based on their “risk”.
