...
data.table::as.data.table(BrainCancer) |>
  (\(DT) DT[, .N,
...
The Weibull model produces a survival curve that is not a step function but it is smooth. It's like using a linear model instead of using a histogram, as you need to put in some assumptions about the distribution, but it is helpful for more complex analyses as
