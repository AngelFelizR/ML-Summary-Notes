...
- *Terminal nodes* or *leaves* are represented by $R_1$, $R_2$ and $R_3$.
...
As result, we could end with a very complex tree that overfits the data. To solve this, we need **prune** the original tree until getting a **subtree** that leads to the lowest test error rate, but rather than using cross-validation to *consider every possible subtree* we can use the **cost complexity pruning** approach some pruned subtrees based on some values of $\alpha$.
...
Then we can use K-fold cross-validation to identify the best $\alpha$ and select a *subtree* created with that $\alpha$ value.
